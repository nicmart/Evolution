...
trait ListAlgebra[F[_], S[_]] {
  ...
  def mapCons[A, B](fa: F[A])(f: SFFunc2[A, B]): F[B]
  def flatMap[A, B](fa: F[A])(f: SFFunc1[A, B]): F[B]
  def map[A, B](f: SFunc1[A, B]): FFunc1[A, B] = ???
  def fix[A](f: FFunc1[A, A]): F[A]
  def fix2[A, B, C](f: HFunc2[A, B, C]): F[A] => F[B] => F[C] = ???
}
...
object UnitInterpreter extends ListAlgebra[ConstUnit, ConstUnit] {
  ...
  override def flatMap[A, B](fa: Unit)(f: SFFunc1[A, B]): Unit = ()
  override def fix[A](f: FFunc1[A, A]): Unit = ()
  override def mapEmpty[A](fa: Unit)(fa2: Unit): Unit = ()
  override def mapCons[A, B](fa: Unit)(f: SFFunc2[A, B]): Unit = ()
}
...
object LazyStreamInterpreter extends ListAlgebra[LazyStream, Id] {
  ...
  override def fix[A](f: A FFunc1 A): LazyStream[A] = f.run(this)(() => fix(f)())
  override def value[A](a: A): Id[A] = a
  override def flatMap[A, B](fa: LazyStream[A])(f: SFFunc1[A, B]): LazyStream[B] =
  ...
  override def mapCons[A, B](fa: LazyStream[A])(f: SFFunc2[A, B]): LazyStream[B] =
    () => {
      val fas = fa()
      fas match {
...
object StringInterpreter extends ListAlgebra[StringConst, StringConst] {
  ...
  override def fix[A](f: A FFunc1 A): String = s"fix($$self => ${f.run(StringInterpreter)("$self")})"
  override def value[A](a: A): String = a.toString
  override def flatMap[A, B](fa: String)(f: SFFunc1[A, B]): String =
    s"flatMap($fa)(x => ${f.run(this)("x")})"
  override def mapEmpty[A](fa: String)(fa2: String): String =
    s"mapEmpty($fa)($fa2)"
  override def mapCons[A, B](fa: String)(f: SFFunc2[A, B]): String =
...
class TwoWayMapLang[F1[_], F2[_], S[_]](map: TwoWayMap[F1, F2], alg: ListAlgebra[F1, S]) extends ListAlgebra[F2, S] {
  ...
  override def flatMap[A, B](fa: F2[A])(f: SFFunc1[A, B]): F2[B] =
    map.to(alg.flatMap(map.from(fa))(f))
  override def fix[A](f: FFunc1[A, A]): F2[A] =
    map.to(alg.fix(f))
  override def mapEmpty[A](fa: F2[A])(fa2: F2[A]): F2[A] =
    map.to(alg.mapEmpty(map.from(fa))(map.from(fa2)))
  override def mapCons[A, B](fa: F2[A])(f: SFFunc2[A, B]): F2[B] =
...
object MapEmpty {
  ...
  class Annotator[F[_], S[_]](alg: ListAlgebra[F, S])
    ...
    override def flatMap[A, B](fa: Annotation[F, A])(f: SFFunc1[A, B]): Annotation[F, B] =
      fa match {
        case Empty() => Empty()
        case _ if EmptyChecker.isEmpty(f) => Empty()
        case _ => super.flatMap(fa)(f)
      }
    override def fix[A](f: A FFunc1 A): Annotation[F, A] =
    ...
    override def mapCons[A, B](fa: Annotation[F, A])(f: SFFunc2[A, B]): Annotation[F, B] =
      fa match {
        case Empty() => Empty()
        case _ if EmptyChecker.isEmpty(f) => Empty()
  ...
  object EmptyChecker extends Annotator[ConstUnit, ConstUnit](UnitInterpreter) {
    def isEmpty[A, B](f: SFFunc1[A, B]): Boolean =
      f.run(this)(Unknown[ConstUnit, A](())).isEmpty
    def isEmpty[A, B](f: FFunc1[A, B]): Boolean =
      f.run(this)(Unknown[ConstUnit, A](())).isEmpty
    def isEmpty[A, B](f: SFFunc2[A, B]): Boolean =
      f.run(this)((), Unknown[ConstUnit, A](())).isEmpty
  }
}
