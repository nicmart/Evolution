...
object segments extends LegacyDrawingDefinition[Point] {
  val name = "segments"

  case class Config(startingSpeed: Double, acceleration: Double, friction: Double, lengthOverSpeed: Int)

  def initialConfig = Config(startingSpeed = 3, acceleration = 0.1, friction = 0.00001, lengthOverSpeed = 100)
    ...
    new LegacyEvolution[Point] {
      override def run[Evo[+ _]](implicit alg: FullAlgebra[Evo]): Evo[Point] = {
        ...
        def accelerationEvolution: Evo[AccelerationLaw[Point]] = {
          rectangle2D(acceleration) map { randomAcc => (position, velocity) =>
            randomAcc - velocity * friction
          }
        }

        solve2(Point(context.left, 0), Point(startingSpeed, 0))(accelerationEvolution).flatMap {
