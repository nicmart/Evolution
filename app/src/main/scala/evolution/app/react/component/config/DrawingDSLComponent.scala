...
object DrawingDSLComponent {
  def apply(component: ConfigComponent[String]): ConfigComponent[DrawingExpr[Point]] =
    instance("drawing config") { (props, children) =>
      val stringSnapshot = props.zoomState[String](drawing => drawing.run(Serializer)(Nil))(
        serialized =>
          currDrawing => PointDrawingParser.parse(serialized).fold(error => { println(error); currDrawing }, identity)
      )
