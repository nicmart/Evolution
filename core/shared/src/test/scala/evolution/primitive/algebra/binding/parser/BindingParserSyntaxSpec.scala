  ...
  "A Binding Algebra Parser should parse" - {
    "let expressions that are " - {
      "simple" in {
        ...
        val expected = let[Double, Double]("x", double(10.0), var0[Double]("x"))
      ...
      "nested" in {
        ...
        val expected =
          let("x", let("y", double(1.0), var0("y")), let("z", double(2.0), shift(var0("x"))))
        unsafeParseDouble(serializedExpression) shouldBe expected.infer(doubleConstant).toString
      }
      ...
      "multi-nested" in {
        ...
        val expected =
          let("x", let("y", double(1.0), var0("y")), let("z", let("u", var0("z"), var0("u")), shift(var0("z"))))
        unsafeParseDouble(serializedExpression) shouldBe expected.infer(doubleConstant).toString
      }

      "inside a lambda" in {
        val serializedExpression = "app(x -> let(y, 1.0, $x), 1.0)"
        val expected = app(lambda("x", let("y", double(1.0), shift(var0("y")))), double(1.0))
        unsafeParseDouble(serializedExpression) shouldBe expected.infer(doubleConstant).toString
      }
    }
    ...
    "lambda expressions that are" - {
      ...
      "nested" in {
        ...
        val expected =
          lambda("x", lambda("y", shift(var0("y"))))
      ...
      "applications of HO lambdas" in {
        ...
        val expected =
          app(lambda[Double, Double => Double]("x", lambda[Double, Double]("y", shift(var0[Double]("y")))), double(1.0))
        unsafeParseLambda(serializedExpression) shouldBe expected.infer(lambdaType).toString
      }

      "inside let expressions" in {
        val serializedExpression = "let(x, 1.0, y -> $x)"
        val expected = let("x", double(1.0), lambda("y", shift(var0("y"))))
        unsafeParseLambda(serializedExpression) shouldBe expected.infer(lambdaType).toString
      }
    }
    ...
    "fix expressions that are" - {
      ...
      "identities" in {
        ...
        val expected = fix(lambda("x", var0("x")))
        unsafeParseDouble(serializedExpression) shouldBe expected.infer(doubleConstant).toString
      }
      ...
      "fixed points of HOF" in {
        ...
        val expected =
          fix(lambda[Double => Double, Double => Double]("f", var0("f")))
        unsafeParseDouble(serializedExpression) shouldBe expected.infer(doubleConstant).toString
      }
    }
  }
