...
class SerializerSpec extends WordSpec with Matchers with PropertyChecks {

  import Builder._

  "A Serializer interpreter" should {
    "serialize a complex drawing" in {
      val drawing = integrate[Point](Point.zero, point(rnd(const(-1), const(1)), rnd(const(-1), const(1))))
    ...
    "serialize a choose expression" in {
      val drawing = choose[Point](const(0.5), const(Point.zero), point(const(0), rnd(const(-1), const(1))))
