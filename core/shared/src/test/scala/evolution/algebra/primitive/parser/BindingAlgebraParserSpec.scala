...
class BindingAlgebraParserSpec extends FreeSpec with Matchers with CommonTestParsers {
  ...
  private lazy val bindingParser: BindingAlgebraParser[CtxString] =
    new BindingAlgebraParser[CtxString](BindingAlgebraSerializer)
  ...
  object Container {
    ...
    implicit val hasDouble: HasParser[Container, CtxString, Double] =
      HasParser.instance(_.double, (c, p) => c.copy(double = p))
  ...
  import bindingParser.hasFunctions
