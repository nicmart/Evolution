...
import evolution.primitive.algebra.parser.{ExtParser, ExtensibleParser, HOExtensibleParser, ParserConfig}
...
class HOExtensibleParserSpec extends WordSpec with Matchers {
  ...
  def additionParser(innerParser: HOExtensibleParser[Expression]): Parser[Expression[Double]] =
    P(function2("add", innerParser.doubleExpr, innerParser.doubleExpr).map {
      case (expr1, expr2) =>
        Add(expr1, expr2)
    })

  def concatParser(innerParser: HOExtensibleParser[Expression]): Parser[Expression[String]] =
    P(function2("concat", innerParser.stringExpr, innerParser.stringExpr).map {
      case (expr1, expr2) =>
        Concat(expr1, expr2)
    })

  def lengthParser(innerParser: HOExtensibleParser[Expression]): Parser[Expression[Double]] =
    function1("length", innerParser.stringExpr).map(Length)

  def toStringParser(innerParser: HOExtensibleParser[Expression]): Parser[Expression[String]] =
  ...
  lazy val extensibleAddition: ExtParser[Double, Expression] =
    ExtParser(doubleParser, additionParser)
  lazy val extensibleConcatenation: ExtParser[String, Expression] =
    ExtParser(stringParser, concatParser)
  lazy val extensibleLengthParser: ExtParser[Double, Expression] =
    ExtParser(doubleParser, lengthParser)
  lazy val extensibleToStringParser: ExtParser[String, Expression] =
    ExtParser(stringParser, toStringParser)

  lazy val extensibleLetBindingForNumToNum: ExtParser[Double, Expression] =
    ExtParser(Fail, self => letSyntax(self.doubleExpr, name => addVarNameForNum(name, self).doubleExpr))
  lazy val extensibleLetBindingForNumToString: ExtParser[String, Expression] =
    ExtParser(Fail, self => letSyntax(self.doubleExpr, name => addVarNameForNum(name, self).stringExpr))
  lazy val extensibleLetBindingForStringToNum: ExtParser[Double, Expression] =
    ExtParser(Fail, self => letSyntax(self.stringExpr, name => addVarNameForString(name, self).doubleExpr))
  lazy val extensibleLetBindingForStringToString: ExtParser[String, Expression] =
  ...
  def addVarNameForNum[A](
    name: String,
    extensibleParser: HOExtensibleParser[Expression]
  ): HOExtensibleParser[Expression] =
    HOExtensibleParser(
      extensibleParser.double.mapLeaf(leaf => leaf.map(expr => VarS(expr)) | var0(name)),
      extensibleParser.string
    )

  def addVarNameForString[A](
    name: String,
    extensibleParser: HOExtensibleParser[Expression]
  ): HOExtensibleParser[Expression] =
    HOExtensibleParser(
