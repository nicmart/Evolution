...
class ExtensibleParserSpec extends WordSpec with Matchers {
  ...
  "An extensible parser" should {
    "parse a simple add expression" in {
      ...
      unsafeParse(serializedExpression, extensibleAddition.expr) shouldBe expectedExpression
    }
    ...
    "parse a combined add and mul expression" in {
      ...
      unsafeParse(serializedExpression, extensibleAdditionWithMultiplication.expr) shouldBe expectedExpression
    }
    ...
    "parse an add expression with a single variable" in {
      ...
      unsafeParse(serializedExpression, addVarName("x", extensibleAddition).expr) shouldBe expectedExpression
    }
    ...
    "parse an add expression with multiple variables" in {
      ...
      unsafeParse(serializedExpression, extensibleParser.expr) shouldBe expectedExpression
    }
  ...
  lazy val doubleExtensibleParser: ExtensibleParser[NumberExpression] = ExtensibleParser(doubleParser)

  def additionParser(innerParser: ExtensibleParser[NumberExpression]): Parser[NumberExpression] =
    P(function2("add", innerParser.expr, innerParser.expr).map {
      case (expr1, expr2) =>
        Add(expr1, expr2)
    })

  def mulParser(innerParser: ExtensibleParser[NumberExpression]): Parser[NumberExpression] =
  ...
  lazy val extensibleAdditionWithMultiplication: ExtensibleParser[NumberExpression] =
    extensibleAddition.extendWith(extensibleMultiplication)
  lazy val extensibleLetBinding: ExtensibleParser[NumberExpression] =
    ExtensibleParser(Fail, self => letSyntax(self.expr, name => addVarName(name, self).expr))
  ...
  def addVarName(
    name: String,
    extensibleParser: ExtensibleParser[NumberExpression]
  ): ExtensibleParser[NumberExpression] =
