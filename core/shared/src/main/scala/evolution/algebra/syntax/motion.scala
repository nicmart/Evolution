...
trait MotionEvolutionSyntax {
  ...
  implicit final def accelerationLawSyntax[Evo[+ _], A](
    evo: Evo[AccelerationLaw[A]]
  ): AccelerationLawEvolutionOps[Evo, A] =
...
final class MotionEvolutionOps[Evo[+ _], A](val evo: Evo[A]) extends AnyVal {
  ...
  def drawAlong(
    other: Evo[Point]
  )(implicit E: MotionEvolutionAlgebra[Evo], ev: Evo[A] =:= Evo[PhaseSpace[Point]]): Evo[Point] =
    E.drawOnEvolution(evo, other)
}

final class PhaseSpaceEvolutionOps[Evo[+ _], A](val evo: Evo[PhaseSpace[A]]) extends AnyVal {
  def positional[B](implicit E: LegacyEvolutionAlgebra[Evo]): Evo[A] =
    E.map(evo)(_._1)
}
...
final class AccelerationLawEvolutionOps[Evo[+ _], A](val evo: Evo[AccelerationLaw[A]]) extends AnyVal {
  def +(other: Evo[AccelerationLaw[A]])(
    implicit
    E: LegacyEvolutionAlgebra[Evo],
    sg: Semigroup[A]
  ): Evo[AccelerationLaw[A]] = {
    E.zipWith(evo, other) { (law1, law2) => (position, velocity) =>
      sg.combine(law1(position, velocity), law2(position, velocity))
    }
  }
}
