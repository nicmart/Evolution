object Test {
  ...
  object Expr {
    ...
    object Chain {
      ...
      case class Cons[T](head: Expr[T], tail: Expr[List[T]]) extends Expr[List[T]]
  ...
  val simpleList: Expr[List[Int]] = Cons(Number(12), Cons(Number(13), Empty()))
  val mappedSimpleList: Expr[List[Int]] = Map(IntFunc(_ + 1), simpleList)

  val nestedList = Cons(simpleList, Cons(simpleList, Empty()))
  val mappedNestedList = Map(Head[Int](), nestedList)
}
