...
class ChainExpr[F[_]] extends Chain[F, Expr[F, ?]] {
  ...
  override def empty[A]: Expr[F, F[A]] =
    new Expr[F, F[A]] {
      override def run[R[_]](alg: Evolution[F, R, String, String]): R[F[A]] = alg.chain.empty
    }

  override def cons[A](head: Expr[F, A], tail: Expr[F, F[A]]): Expr[F, F[A]] =
    new Expr[F, F[A]] {
      override def run[R[_]](alg: Evolution[F, R, String, String]): R[F[A]] =
  ...
  override def mapCons[A, B](eva: Expr[F, F[A]])(f: Expr[F, A => F[A] => F[B]]): Expr[F, F[B]] =
    new Expr[F, F[B]] {
      override def run[R[_]](alg: Evolution[F, R, String, String]): R[F[B]] =
