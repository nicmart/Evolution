...
class ChainBySize[S[_], F[_], R[_]](alg: Chain[S, F, R], val orMonoid: MonoidK[R])
    extends Chain[S, F, Sized[R, ?]]
    with OrMonoid[R] {
  override def empty[A]: Sized[R, F[A]] =
    size => if (size == 0) alg.empty else orMonoid.empty

  override def cons[A](head: Sized[R, S[A]], tail: Sized[R, F[A]]): Sized[R, F[A]] =
  ...
  override def mapCons[A, B](eva: Sized[R, F[A]])(f: Sized[R, S[A] => F[A] => F[B]]): Sized[R, F[B]] =
    size => withSize[F[A], S[A] => F[A] => F[B], F[B]](size - 1, eva, f, (a, b) => alg.mapCons(a)(b))
