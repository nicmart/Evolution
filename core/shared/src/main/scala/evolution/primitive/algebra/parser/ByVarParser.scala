...
sealed trait ByVarParser[T] {
  ...
  def parser(vars: List[String]): Parser[T] = this match {
    case ByVarParser.Fail()     => noApi.Fail
    case Pure(t)                => noApi.PassWith(t)
    case Raw(f, label)          => f(vars)
    case Prefixed(prefix, next) => P(prefix ~ next.parser(vars))
    case Or(parsers)            => Either(Either.flatten(parsers.map(_.parser(vars)).toVector): _*)
  }

  def flatMap[B](f: T => ByVarParser[B]): ByVarParser[B] = this match {
    case ByVarParser.Fail()     => ByVarParser.Fail()
    case Pure(t)                => f(t)
    case Raw(g, label)          => Raw(vars => g(vars).flatMap(t => f(t).parser(vars)), s"$label flatMapped")
    case Prefixed(prefix, next) => Prefixed(prefix, next.flatMap(f))
    case Or(parsers)            => Or(parsers.map(_.flatMap(f)))
  }
  ...
  def loggingParser(vars: List[String]): Parser[T] = this match {
    case ByVarParser.Fail() => parser(vars)
    case Pure(t)            => parser(vars)
    case Raw(f, label)      => debugParser(s"Raw $label: ($vars)", parser(vars))
    case Prefixed(prefix, next) =>
      P(debugParser(s"$prefix ($vars)", prefix) ~ debugParser(s"Suffix of $prefix ($vars)", next.loggingParser(vars)))
    case Or(parsers) =>
      val loggingParsers = parsers.map(_.loggingParser(vars))
      debugParser(
        s"OR (${vars.mkString(", ")}) of (${parsers.mkString(", ")})",
        Either(Either.flatten(loggingParsers.toVector): _*)
      )
  }
}
