...
trait ExpressionModule[F[_]] {
  ...
  object Expr {
    ...
    final case class Integrate[A](start: Expr[A], speed: Expr[F[A]], group: Group[A])
        extends Expr[F[A]](List(start, speed))
    final case class Solve1[A](speed: Expr[F[A => A]], start: Expr[A], group: Group[A])
        extends Expr[F[A]](List(speed, start))
    final case class Solve2[A](acc: Expr[F[A => A => A]], a0: Expr[A], v0: Expr[A], group: Group[A])
    ...
    final case class WithFirst3[A, B](as: Expr[F[A]], f: Expr[A => A => A => F[B]]) extends Expr[F[B]](List(as, f))
