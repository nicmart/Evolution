...
import evolution.compiler.term.Term.{Lambda, *}
...
class CaptureAvoidingRenamerTest extends LanguageSpec:
  ...
  "x -> x' -> x" in {
    ...
    val Lambda(var1, Lambda(var2, Id(id2))) = rename(term): @unchecked
  ...
  "x -> x -> x' -> x" in {
    ...
    val Lambda(var1, Lambda(var2, Lambda(var3, Id(id2)))) = rename(term): @unchecked
  ...
  "rename lambda bindings" in {
    ...
    val Lambda(var1, Lambda(var2, Id(idLambda2))) = renamed: @unchecked
    Set(var1, var2) `should` have `size` (2)
  ...
  "rename let bindings" in {
    ...
    val Let(x1, Id(id0), Let(x2, Id(id1), Id(id2))) = renamed: @unchecked
