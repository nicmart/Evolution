...
trait PaintEvolutionSpec extends WordSpec with Matchers {
  ...
  import EvolutionLegacy._

  def assertEquivalent[A](ev1: EvolutionLegacy[A], ev2: EvolutionLegacy[A], size: Int = 50): Unit = {
  ...
  def assertEvolution[A](ev: EvolutionLegacy[A], expected: List[(RNG, A)]): Unit = {
    val rng = SequenceRNG(0)
    val sample = debug(ev, expected.length)(Some(rng))
    sample shouldBe expected
  }

  def assertSameValues[A](ev1: EvolutionLegacy[A], ev2: EvolutionLegacy[A], size: Int = 50): Unit = {
    val rng = SequenceRNG(0)
  ...
  def assertValues[A](ev: EvolutionLegacy[A], expected: List[A]): Unit = {
    val rng = SequenceRNG(0)
    val sample = EvolutionLegacy.sample(ev, expected.length, Some(rng))
    sample shouldBe expected
  }
}
