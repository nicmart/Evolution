...
object Evolution:
  ...
  def distinct[T](evo: Evolution[T], n: Int): Evolution[T] = new Evolution[T]:
    override def run: Iterator[T] = evo.run.take(n).toList.distinct.iterator
              ...
              if outerIterationEnded then
                if iterators(currentIndex).hasNext then true
                  ...
                  if iterators.isEmpty then false
                    ...
                    hasNext
              else if iteratorOfEvolutions.hasNext then
  ...
  private def smoothModule(n: Double, b: Int): Double =
    if n >= 0 then if ((n / b).toInt % 2 == 0) then positiveModule(n, b) else b - positiveModule(n, b) - 1
    else if ((n / b).toInt % 2 == 1) then positiveModule(n, b)
    else b - positiveModule(n, b) - 1
