...
abstract sealed class Constant0(qualifiedScheme: Qualified[Scheme])
    extends Constant(qualifiedScheme)
    with EnumEntry
    with Lowercase {
...
object Constant0 extends Enum[Constant0] {
  ...
  case object PI extends Constant0(Qualified(Scheme(Type.Double))) {
    def compile(tpe: Qualified[Type]): Either[String, Expr[_]] =
      Expr.Dbl(Math.PI).asRight
  }

  case object Empty extends Constant0(Qualified(Scheme(List("T"), Var("T")))) {
    def compile(tpe: Qualified[Type]): Either[String, Expr[_]] =
      Expr.FromList(Expr.Lst(Nil)).asRight
  }

  case object Noise extends Constant0(Qualified(Scheme(Evo(Type.Point =>: Type.Double)))) {
    override def compile(tpe: TypeClasses.Qualified[Type]): Either[String, Expr[_]] =
      Expr.Noise().asRight
  }

  case object OctaveNoise
      extends Constant0(Qualified(Scheme(Evo(Integer =>: Type.Double =>: Type.Point =>: Type.Double)))) {
    override def compile(tpe: TypeClasses.Qualified[Type]): Either[String, Expr[_]] =
...
abstract sealed class Constant1(qualifiedScheme: Qualified[Scheme])
    extends Constant(qualifiedScheme)
    with EnumEntry
    with Lowercase {
...
abstract sealed class Constant1Plain(qualifiedScheme: Qualified[Scheme]) extends Constant1(qualifiedScheme) {
...
object Constant1 extends Enum[Constant1] {
  ...
  case object X extends Constant1Plain(Qualified(Scheme(Type.Point =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.X(x.asExpr)
  }

  case object Y extends Constant1Plain(Qualified(Scheme(Type.Point =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Y(x.asExpr)
  }

  case object Floor extends Constant1Plain(Qualified(Scheme(Type.Double =>: Integer))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Floor(x.asExpr)
  }

  case object ToDbl extends Constant1Plain(Qualified(Scheme(Integer =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.ToDouble(x.asExpr)
  }

  case object Abs extends Constant1Plain(Qualified(Scheme(Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Abs(x.asExpr)
  }

  case object Sign extends Constant1Plain(Qualified(Scheme(Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Sign(x.asExpr)
  }

  case object Norm extends Constant1Plain(Qualified(Scheme(Type.Point =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Norm(x.asExpr)
  }

  case object Versor extends Constant1Plain(Qualified(Scheme(Type.Point =>: Type.Point))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Versor(x.asExpr)
  }

  case object Inverse extends Constant1(Qualified(Scheme(List("T"), Var("T") =>: Var("T")))) {
    override def compile(x: Typed[_], out: Type): Either[String, Expr[_]] =
      TypingConfig.invertible(x.tpe).map(inv => Expr.Inverse(x.value, inv))
  }

  case object Sin extends Constant1Plain(Qualified(Scheme(Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Sin(x.asExpr)
  }

  case object Cos extends Constant1Plain(Qualified(Scheme(Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Cos(x.asExpr)
  }

  case object Not extends Constant1Plain(Qualified(Scheme(Bool =>: Bool))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Not(x.asExpr)
  }

  case object Constant extends Constant1Plain(Qualified(Scheme(List("T"), Var("T") =>: Evo(Var("T"))))) {
    override def entryName: String = "const"
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Constant(x.asExpr)
  }

  case object FromList extends Constant1Plain(Qualified(Scheme(List("T"), Lst(Var("T")) =>: Evo(Var("T"))))) {
    def compilePlain(x: Expr[_]): Expr[_] = Expr.FromList(x.asExpr)
  }

  case object Fix extends Constant1Plain(Qualified(Scheme(List("T"), (Var("T") =>: Var("T")) =>: Var("T")))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Fix(x.asExpr[Any => Any])
  }

  case object Flatten extends Constant1Plain(Qualified(Scheme(List("T"), Evo(Evo(Var("T"))) =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Flatten(x.asExprF[Evolution[Any]])
  }

  case object Parallel extends Constant1Plain(Qualified(Scheme(List("T"), Evo(Evo(Var("T"))) =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_]): Expr[_] = Expr.Parallel(x.asExprF[Evolution[Any]])
  }

  case object Derive
      extends Constant1(Qualified(isInvertSemigroup("T"), Scheme(List("T"), Evo(Var("T")) =>: Evo(Var("T"))))) {
  ...
  case object UniformChoice extends Constant1Plain(Qualified(Scheme(List("T"), Lst(Var("T")) =>: Evo(Var("T"))))) {
...
abstract sealed class Constant2(qualifiedScheme: Qualified[Scheme])
    extends Constant(qualifiedScheme)
    with EnumEntry
    with Lowercase {
...
object Constant2 extends Enum[Constant2] {
  ...
  case object Point extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Point))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Pnt(x.asExpr, y.asExpr)
  }

  case object LiftedPoint
      extends Constant2Plain(Qualified(Scheme(Evo(Type.Double) =>: Evo(Type.Double) =>: Evo(Type.Point)))) {
  ...
  case object Polar extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Point))) {
  ...
  case object Minus
      extends Constant2(Qualified(isInvertSemigroup("T"), Scheme(List("T"), Var("T") =>: Var("T") =>: Var("T")))) {
    override def compile(x: Typed[_], y: Typed[_], out: Type): Either[String, Expr[_]] =
      for {
  ...
  case object Div extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Div(x.asExpr, y.asExpr)
  }

  case object Exp extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Exp(x.asExpr, y.asExpr)
  }

  case object Mod extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Double))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Mod(x.asExpr, y.asExpr)
  }

  case object And extends Constant2Plain(Qualified(Scheme(Bool =>: Bool =>: Bool))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.And(x.asExpr, y.asExpr)
  }

  case object Or extends Constant2Plain(Qualified(Scheme(Bool =>: Bool =>: Bool))) {
  ...
  case object Cons extends Constant2Plain(Qualified(Scheme(List("T"), Var("T") =>: Evo(Var("T")) =>: Evo(Var("T"))))) {
  ...
  case object WithFirst
      extends Constant2Plain(
        Qualified(Scheme(List("T1", "T2"), Evo(Var("T1")) =>: (Var("T1") =>: Evo(Var("T2"))) =>: Evo(Var("T2"))))
      ) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] =
  ...
  case object Connect
      extends Constant2Plain(
        Qualified(Scheme(List("T"), Evo(Var("T")) =>: (Var("T") =>: Evo(Var("T"))) =>: Evo(Var("T"))))
      ) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] =
      Expr.Connect(x.asExprF, y.asExpr[Any => Evolution[Any]])
  }

  case object Integrate extends Constant2(Qualified(Scheme(List("T"), Var("T") =>: Evo(Var("T")) =>: Evo(Var("T"))))) {
    override def compile(x: Typed[_], y: Typed[_], out: Type): Either[String, Expr[_]] =
      TypingConfig.additive(x.tpe, x.tpe, x.tpe).map(add => Expr.Integrate(x.value, y.value.asExprF, add))
  }

  case object Solve1
      extends Constant2(Qualified(Scheme(List("T"), Evo(Var("T") =>: Var("T")) =>: Var("T") =>: Evo(Var("T"))))) {
    override def compile(x: Typed[_], y: Typed[_], out: Type): Either[String, Expr[_]] =
      TypingConfig.additive(y.tpe, y.tpe, y.tpe).map(add => Expr.Solve1(x.value.asExprF, y.value, add))
  }

  case object Roll
      extends Constant2Plain(Qualified(Scheme(List("T"), Evo(Var("T") =>: Var("T")) =>: Var("T") =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] =
      Expr.Roll(x.asExprF[Any => Any], y)
  }

  case object Concat
      extends Constant2Plain(Qualified(Scheme(List("T"), Evo(Var("T")) =>: Evo(Var("T")) =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Concat(x.asExprF, y.asExprF)
  }

  case object Map
      extends Constant2Plain(
        Qualified(Scheme(List("T1", "T2"), Evo(Var("T1")) =>: (Var("T1") =>: Var("T2")) =>: Evo(Var("T2"))))
      ) {
  ...
  case object SlidingMap
      extends Constant2Plain(
        Qualified(
          Scheme(List("T1", "T2"), Evo(Var("T1")) =>: (Var("T1") =>: Var("T1") =>: Var("T2")) =>: Evo(Var("T2")))
        )
      ) {
  ...
  case object Iterate
      extends Constant2Plain(Qualified(Scheme(List("T"), (Var("T") =>: Var("T")) =>: Var("T") =>: Evo(Var("T"))))) {
  ...
  case object Take extends Constant2Plain(Qualified(Scheme(List("T"), Evo(Var("T")) =>: Integer =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Take(x.asExprF, y.asExpr)
  }

  case object Grouped
      extends Constant2Plain(Qualified(Scheme(List("T"), Evo(Var("T")) =>: Integer =>: Evo(Lst(Var("T")))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Grouped(x.asExprF, y.asExpr)
  }

  // syntactic sugar
  case object Filter
      extends Constant2Plain(Qualified(Scheme(List("T"), Evo(Var("T")) =>: (Var("T") =>: Bool) =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = {
  ...
  case object Uniform extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Evo(Type.Double)))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.Uniform(x.asExpr, y.asExpr)
  }

  case object UniformFrom
      extends Constant2Plain(Qualified(Scheme(List("T"), Integer =>: Evo(Var("T")) =>: Evo(Var("T"))))) {
    override def compilePlain(x: Expr[_], y: Expr[_]): Expr[_] = Expr.UniformFrom(x.asExpr, y.asExprF)
  }

  case object Normal extends Constant2Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Evo(Type.Double)))) {
...
abstract sealed class Constant3(qualifiedScheme: Qualified[Scheme])
    extends Constant(qualifiedScheme)
    with EnumEntry
    with Lowercase {
...
abstract sealed class Constant3Plain(qualifiedScheme: Qualified[Scheme]) extends Constant3(qualifiedScheme) {
...
object Constant3 extends Enum[Constant3] {
  ...
  case object SmoothStep
      extends Constant3Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Double =>: Type.Double))) {
  ...
  case object Range
      extends Constant3Plain(Qualified(Scheme(Type.Double =>: Type.Double =>: Type.Double =>: Evo(Type.Double)))) {
  ...
  case object Solve2
      extends Constant3(
        Qualified(
          Scheme(List("T"), Evo(Var("T") =>: Var("T") =>: Var("T")) =>: Var("T") =>: Var("T") =>: Evo(Var("T")))
        )
      ) {
  ...
  case object Iterate2
      extends Constant3Plain(
        Qualified(Scheme(List("T"), (Var("T") =>: Var("T") =>: Var("T")) =>: Var("T") =>: Var("T") =>: Evo(Var("T"))))
      ) {
    def compilePlain(f: Expr[_], a0: Expr[_], a1: Expr[_]): Expr[_] =
      Expr.Iterate2(f.asExpr, a0.asExpr, a1.asExpr)
  }

  case object InRect extends Constant3Plain(Qualified(Scheme(Type.Point =>: Type.Point =>: Type.Point =>: Bool))) {
    override def compilePlain(x: Expr[_], y: Expr[_], z: Expr[_]): Expr[_] =
      Expr.InRect(x.asExpr[Point], y.asExpr[Point], z.asExpr[Point])
  }
  case object If extends Constant3Plain(Qualified(Scheme(List("T"), Bool =>: Var("T") =>: Var("T") =>: Var("T")))) {
    override def compilePlain(x: Expr[_], y: Expr[_], z: Expr[_]): Expr[_] =
